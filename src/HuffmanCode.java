...
public class HuffmanCode{
    ...
    public static void ouputHaffmanCode(Node[] huffmanTree,NodeCode[] huffmanCode,int n){
        ...
        int sum = 0;
        for(int i=0;i<n;i++){
            sum += huffmanCode[i].getCode().length() * huffmanTree[i].getWeight();
    ...
    public static void main(String[] args){
        ...
        //设置huffmanCode的符号以及出现频次
        int mn = 0;
        for(int i=0;i<26;i++){
            if(count[i] != 0) {
                huffmanCode[mn] = new NodeCode(String.valueOf((char)('a' + i)), "");
                huffmanTree[mn] = new Node(count[i], -1, -1, -1);
                mn++;
            }
        }
